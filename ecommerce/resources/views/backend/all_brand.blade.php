...
<div class="d-flex justify-content-end mt-20 toast_message" role="alert" aria-live="assertive" aria-atomic="true">
    ...
    style="opacity: 1 ">
      <div class="toast-body">
        {{ Session::get('success_brand') }}
      </div>
    </div>
</div>
@endif

@if (Session::has('success_delete_brand'))
<div class="d-flex justify-content-end mt-20 toast_message" role="alert" aria-live="assertive" aria-atomic="true">
    <div class=" toast d-flex align-items-center text-white bg-success border-0s"
    style="opacity: 1 ">
      <div class="toast-body">
        {{ Session::get('success_delete_brand') }}
      </div>
    </div>
</div>
@endif
    ...
    <a href="{{ route('brand.create') }}" class="btn btn-info float-right white">Add brand</a>
                    ...
                    <tbody>
                        @foreach ($data as $index => $each )
                            <tr>
                                ...
                                <td>
                                    <img src="{{ url($each->brand_image) }}" alt="" style="width: 60px; height: 60px;">
                                </td>
                                <td><a href="{{ route('brand.edit', $each) }}" class="btn btn-primary">Edit</a></td>
                                <td><a href="{{ route('brand.destroy', $each) }}" class="btn btn-danger">Delete</a></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
