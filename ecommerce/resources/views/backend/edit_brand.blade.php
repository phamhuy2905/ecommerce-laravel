...
<div class="card-content">
    <div class="card-body pt-1">
        <form method="POST" action="{{ route('brand.update') }}" id="form" enctype="multipart/form-data">
            @csrf
            <input type="text" value="{{ $brand->id }}" hidden name="id">
            <input type="text" value="{{ $brand->brand_image }}" hidden name="brand_image_old">
            <div class="form-label-group form-group position-relative has-icon-left">
                <label for="name">Brand name</label>
                <input class="form-control" id="name" placeholder="Brand name"  name="brand_name"  
                value="{{ $brand->brand_name }}">
                <span class="message_validate text-danger" style="font-size: 12px"></span>
                @if ($errors->has('brand_name'))
                <span class="message_validate text-danger" style="font-size: 12px">{{ $errors->first('brand_name') }}</span>
                @endif
            </div>
            <div class="media-body mt-75">
                <img src="{{ url($brand->brand_image) }}" class="rounded mr-75 border-primary show_img" alt="profile image" height="64" width="64" style="object-fit: cover" class="show_img" id="show_img">
                <label for="image" class="btn btn-sm btn-primary ml-50 mb-50 mb-sm-0 cursor-pointer" style="opacity: 1">Brand image</label>
                <input class="form-control" id="image" placeholder="Brand image"  name="brand_image" type="file" hidden>
                <span class="message_validate text-danger" style="font-size: 12px"></span>
